...
async function runLocalBuild({ deployId, project, sitesDir, tmpDir, githubToken, emit, onLog }) {
  ...
  // ── Note Start Command (informational for static hosting) ─────────
  const startCmd = project.startCmd || '';
  if (startCmd) {
    log(`\n\x1b[36m━━━ Start Command (recorded) ━━━\x1b[0m`);
    log(`\x1b[90m[info] Start command: \x1b[0m\x1b[1m${startCmd}\x1b[0m`);
    log(`\x1b[90m[info] Note: DeployBoard hosts static output (dist folder). The start command\x1b[0m`);
    log(`\x1b[90m[info] is saved for reference — use it when running this app as a Node.js service.\x1b[0m`);
  }

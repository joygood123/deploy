...
async function runLocalBuild({ deployId, project, sitesDir, tmpDir, githubToken, emit, onLog }) {
  ...
  await exec(installParts[0], installParts.slice(1), {
    ...
    env: {
      ...process.env,
      ...envObj,
      // Use development so devDependencies (typescript, vite, webpack etc.)
      // are installed — they are needed for the build step
  ...
  if (startCmd) {
    ...
    // For Node.js apps (npm start, node server.js etc.) we need a persistent process.
    // We use pm2 if available, otherwise we run and capture startup output for 12s.
    const isNodeServer = /^(npm\s+start|node\s+|yarn\s+start|npx\s+)/.test(startCmd);

    if (isNodeServer) {
      // Check if pm2 is available
      const pm2Available = await new Promise(resolve => {
        const check = spawn('which', ['pm2'], { shell: false });
        check.on('close', code => resolve(code === 0));
        check.on('error', () => resolve(false));
      });

      if (pm2Available) {
        log(`\x1b[90m[start] Using pm2 to keep Node.js app running persistently\x1b[0m`);
        const appName = `deployboard-${project.subdomain}`;
        const startParts = splitCmd(startCmd);
        // Stop any previous instance
        await new Promise(resolve => {
          const stop = spawn('pm2', ['delete', appName], { shell: false });
          stop.on('close', () => resolve());
          stop.on('error', () => resolve());
        });
        // Start with pm2
        try {
          await exec('pm2', ['start', '--name', appName,
            '--cwd', projectRoot,
            '--', ...startParts
          ], { cwd: projectRoot, env: { ...process.env, ...envObj, PORT: process.env.PORT || '3000' } }, log);
          await exec('pm2', ['save'], {}, log);
          log(`\x1b[32m[start] ✓ App started with pm2 as "${appName}"\x1b[0m`);
          log(`\x1b[90m[start] Your Node.js app is running persistently.\x1b[0m`);
          log(`\x1b[90m[start] The wildcard tunnel will route subdomain traffic to it.\x1b[0m`);
        } catch(e) {
          log(`\x1b[33m[start] pm2 start had issues: ${e.message}\x1b[0m`);
        }
      } else {
        // pm2 not available — run for 12s to capture startup output
        log(`\x1b[90m[start] Running start command (capturing first 12s of output)…\x1b[0m`);
        const startParts = splitCmd(startCmd);
        try {
          await execWithTimeout(startParts[0], startParts.slice(1), {
            cwd: projectRoot,
            env: { ...process.env, ...envObj, PORT: process.env.PORT || '3000' }
          }, log, 12000);
        } catch (startErr) {
          if (startErr.message.includes('TIMEOUT')) {
            log(`\x1b[32m[start] ✓ Server started successfully (running in background)\x1b[0m`);
          } else {
            log(`\x1b[33m[start] Warning: ${startErr.message}\x1b[0m`);
            log(`\x1b[33m[start] Your static files are still deployed above.\x1b[0m`);
          }
        }
      }
    } else {
      // Non-server command — just run it directly
      const startParts = splitCmd(startCmd);
      try {
        await execWithTimeout(startParts[0], startParts.slice(1), {
          cwd: projectRoot,

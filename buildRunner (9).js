...
async function runLocalBuild({ deployId, project, sitesDir, tmpDir, githubToken, emit, onLog }) {
  ...
  // For server-type apps, if the output dir doesn't exist, copy the whole project
  // (they don't produce a separate dist/ — they serve directly from project root)
  const isServerApp = (project.siteType === 'server') || !!(project.startCmd || '').trim();
  if (!fs.existsSync(outputDir)) {
    if (isServerApp) {
      log(`\x1b[33m[copy] Output dir "${project.outputDir||'dist'}" not found — this is a server app.\x1b[0m`);
      log(`\x1b[33m[copy] Copying entire project directory so start command can run.\x1b[0m`);
      // For server apps, copy the whole project to the dest dir
      const serverDestDir = path.join(sitesDir, project.subdomain, 'app');
      fs.mkdirSync(serverDestDir, { recursive: true });
      if (fs.existsSync(serverDestDir)) fs.rmSync(serverDestDir, { recursive: true, force: true });
      copyDirSync(projectRoot, serverDestDir);
      emitStep(emit, 'copy', 'done');
      log(`\x1b[32m[copy] Server app copied to ${serverDestDir}\x1b[0m`);
      if (project._assignedPort) {
        log(`\x1b[90m[info] PORT=${project._assignedPort} — Nginx will proxy this port\x1b[0m`);
      }
      log(`\x1b[90m[info] To serve this app, run: ${project.startCmd || 'npm start'} from ${serverDestDir}\x1b[0m`);
      // Skip to start/cleanup — don't throw
      return await runStartAndCleanup({ project, projectRoot, buildDir, emit, log, onLog, envObj });
    }
    ...
    if (pm2Available) {
      ...
      try {
        await execWithTimeout(startParts[0], startParts.slice(1),
          { cwd: projectRoot, env: startEnv }, log, 12000);
      } catch(e) {
        if (e.message.includes('TIMEOUT')) {
          log(`\x1b[32m[start] ✓ Server started (running in background)\x1b[0m`);
        } else {
          log(`\x1b[33m[start] Warning: ${e.message}\x1b[0m`);
        }
      }
    }

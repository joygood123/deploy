...
// Write a per-subdomain Nginx config that proxies to the Node.js app port
async function writeNginxProxyConfig(subdomain, port) {
  const configDir = NGINX_SITES_DIR;
  try { fs.mkdirSync(configDir, { recursive: true }); } catch(e) {}

  const configContent = `# Auto-generated by DeployBoard for server app: ${subdomain}
# Port: ${port}
server {
    listen 80;
...
`;
  ...
  try {
    ...
    const nginxCmd = (() => {
      const candidates = ['/usr/sbin/nginx', '/usr/local/sbin/nginx', '/sbin/nginx', 'nginx'];
      for (const c of candidates) {
        try { execSync(`${c} -v 2>&1`, { stdio:'pipe' }); return c; } catch(e) {}
      }
      return null;
    })();
    if (!nginxCmd) {
      console.warn('[Nginx] nginx binary not found — install with: apt-get install -y nginx');
      return false;
    }
    execSync(`${nginxCmd} -t`, { stdio: 'pipe' });
    execSync(`${nginxCmd} -s reload`, { stdio: 'pipe' });
...
async function removeNginxConfig(subdomain) {
  const configPath = path.join(NGINX_SITES_DIR, `${subdomain}.conf`);
  try {
    if (fs.existsSync(configPath)) {
      fs.rmSync(configPath);
      const { execSync } = require('child_process');
      try { execSync('/usr/sbin/nginx -s reload || nginx -s reload', { stdio: 'pipe' }); }
      catch(e) {}

...
app.use((req, res, next) => {
  ...
  // Helper: serve static files from dist (used as primary and fallback)
  const serveStatic = () => {
    if (!hasDistDir) {
      if (hasAppDir) {
        // Server app folder exists but app not running — show redeploy message
        return res.status(503).send(`<!DOCTYPE html><html><head><title>App Not Running</title>
          <style>body{font-family:sans-serif;background:#060b14;color:#e2e8f0;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;}.box{text-align:center;padding:2rem;}code{background:#1e293b;padding:2px 6px;border-radius:4px;}a{color:#10b981;}</style></head>
          <body><div class="box"><h1>⚠️ App Not Running</h1>
          <p><code>${subdomain}</code> was deployed but is not currently running.</p>
          <p><a href="https://${BASE_DOMAIN}">Redeploy from DeployBoard</a> to restart it.</p>
          </div></body></html>`);
      }
      return res.status(404).send(`<!DOCTYPE html><html><head><title>Not Found</title>
        <style>body{font-family:sans-serif;background:#060b14;color:#e2e8f0;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;}.box{text-align:center;padding:2rem;}code{background:#1e293b;padding:2px 6px;border-radius:4px;}a{color:#10b981;}</style></head>
        <body><div class="box"><h1>404 — Site Not Found</h1>
        <p><code>${subdomain}.${BASE_DOMAIN}</code> has not been deployed yet.</p>
        <p>Deploy it from <a href="https://${BASE_DOMAIN}">DeployBoard</a>.</p>
        </div></body></html>`);
    }
  ...
  // ── Server app: proxy to app port ────────────────────────────────────────
  // Only proxy if the port is registered AND the app directory actually exists.
  // If the app dir is gone (stale registry entry from old deploy), fall through
  // to static serving so static sites aren't broken by stale port entries.
  if (appPort && hasAppDir) {
    ...
    const proxyReq = httpMod.request(options, (proxyRes) => {
      // If the app returns 404 on the root path, show a helpful page
      // instead of the raw JSON {"message":"Cannot GET /","error":"Not Found"}
      if (proxyRes.statusCode === 404 && req.url === '/') {
        let body = '';
        proxyRes.on('data', chunk => body += chunk);
        proxyRes.on('end', () => {
          // Only intercept if it looks like a framework 404 (NestJS/Express)
          const isFramework404 = body.includes('Cannot GET') || body.includes('Not Found');
          if (isFramework404) {
            return res.status(200).send(`<!DOCTYPE html><html><head><title>${subdomain} — Running</title>
              ...
              .box{text-align:center;padding:2rem;}.badge{display:inline-block;background:#10b981;color:#fff;padding:4px 12px;border-radius:20px;font-size:0.85rem;margin-bottom:1rem;}
              code{background:#1e293b;padding:3px 8px;border-radius:4px;font-family:monospace;}
              .hint{color:#94a3b8;font-size:0.9rem;margin-top:1rem;}</style></head>
              <body><div class="box">
              <div class="badge">✓ App is Running</div>
              <h1>${subdomain}</h1>
              <p>Your Node.js app is live and responding on port ${appPort}.</p>
              <p class="hint">This app has no <code>/</code> route.<br>
              Try visiting a specific API endpoint, e.g.:<br>
              <code>/${subdomain}.${BASE_DOMAIN}/api</code><br>
              <code>/${subdomain}.${BASE_DOMAIN}/users</code><br>
              <code>/${subdomain}.${BASE_DOMAIN}/messages</code></p>
              </div></body></html>`);
          }
          // Not a framework 404 — forward the original response
    ...
    proxyReq.on('error', (e) => {
      console.error(`[Proxy] ${subdomain}:${appPort} error: ${e.message}`);
      // App port is registered but not responding — if there's also a dist dir
      // (e.g. after a static redeploy) serve that. Otherwise show 502.
      if (hasDistDir) return serveStatic();
      res.status(502).send(`<!DOCTYPE html><html><head><title>502</title>
        <style>body{font-family:sans-serif;background:#060b14;color:#e2e8f0;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;}.box{text-align:center;padding:2rem;}code{background:#1e293b;padding:2px 6px;border-radius:4px;}a{color:#10b981;}</style></head>
        <body><div class="box"><h1>502 — App Not Responding</h1>
        <p><code>${subdomain}</code> is deployed but not responding on port ${appPort}.</p>
        <p>It may still be starting. Wait a moment and refresh, or <a href="https://${BASE_DOMAIN}">redeploy</a>.</p>
        </div></body></html>`);
    });
  ...
  // Stale port registry entry (app dir deleted) — clean it up and serve static
  if (appPort && !hasAppDir) {
    console.log(`[Ports] Removing stale registry entry for ${subdomain} (app dir gone)`);
    delete portRegistry[subdomain];
    savePortRegistry();
  }

...
app.use((req, res, next) => {
  ...
  // ── Server app: proxy to pm2 port ────────────────────────────────────────
  // If this subdomain has an assigned port in the registry, it's a Node.js
  // server app. Proxy the request to it directly (works even when Cloudflare
  // tunnel points to port 3001 instead of nginx port 80).
  ...
  if (appPort) {
    ...
    proxyReq.on('error', (e) => {
      console.error(`[Proxy] Error proxying ${subdomain} → port ${appPort}:`, e.message);
      res.status(502).send(`
        ...
        <body><div class="box">
          <h1>502 — App Not Responding</h1>
          <p>The app <code>${subdomain}</code> is deployed but not responding on port ${appPort}.</p>
          <p>It may still be starting up. Try again in a moment.</p>
        </div></body></html>
      `);
    });
  ...
  // ── Static site: serve from dist folder ──────────────────────────────────
  const siteDir = path.join(SITES_DIR, subdomain, 'dist');

  if (!fs.existsSync(siteDir)) {
    // Also check the app folder (server apps copied here without dist)
    const appDir = path.join(SITES_DIR, subdomain, 'app');
    if (fs.existsSync(appDir)) {
      // Server app was deployed but port not in registry (restarted?)
      return res.status(503).send(`
        ...
        <body><div class="box">
          ...
          <p><code>${subdomain}</code> was deployed as a server app but is not currently running.</p>
          <p>Redeploy it from <a href="https://${BASE_DOMAIN}" style="color:#10b981;">DeployBoard</a> to restart it.</p>
        </div></body></html>
      `);
    }
    return res.status(404).send(`
      ...
      <body><div class="box">
        ...
        <p>Deploy it from <a href="https://${BASE_DOMAIN}" style="color:#10b981;">DeployBoard</a>.</p>
      </div></body></html>
    `);
  }
...
app.post('/api/deploy', async (req, res) => {
  ...
  try {
    project = await Project.findOneAndUpdate(
      ...
      { name, subdomain: cleanSub, repoUrl,
        ...
        appPort:    ((siteType === 'server') || !!(startCmd || '').trim()) ? getOrAssignPort(cleanSub) : 0,
        envVars:    envVars    || {},
        updatedAt:  new Date() },
  ...
  try {
    ...
    const appPort = (siteType === 'server' || !!(startCmd || '').trim())

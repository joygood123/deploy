...
`;
  ...
  try {
    ...
    execSync('nginx -t 2>/dev/null || sudo nginx -t', { stdio: 'pipe' });
    execSync('nginx -s reload 2>/dev/null || sudo nginx -s reload', { stdio: 'pipe' });
...
async function removeNginxConfig(subdomain) {
  ...
  try {
    if (fs.existsSync(configPath)) {
      ...
      const { execSync } = require('child_process');
      try { execSync('nginx -s reload 2>/dev/null || sudo nginx -s reload', { stdio: 'pipe' }); }
      catch(e) {}
